...
import (
	...
	coregogenerate "myitcv.io/gogenerate"
)

//go:generate gobin -m -run myitcv.io/cmd/internal/gogenerate/internal/genmain
...
const (
	...
	gogeneratePrefix = goGeneratePrefix + ":gogenerate"
)
...
func installGoWrapper() (string, error) {
	td, err := ioutil.TempDir("", "gogenerate_gobin_tmp_")
	if err != nil {
...
func mainerr() (reterr error) {
	...
	artefactsCacheDir := os.Getenv("GOGENERATECACHE")
	if artefactsCacheDir == "" {
		ucd, err := os.UserCacheDir()
		if err != nil {
			return fmt.Errorf("failed to determine user cache dir: %v", err)
		}
		artefactsCacheDir = filepath.Join(ucd, "gogenerate-artefacts")
	}
	...
	td, err := ioutil.TempDir("", "gogenerate-workings")
	if err != nil {
	...
	gogenerate.cliPatts = flagSet.Args()
	gogenerate.mainMod = mm

	if !*fDebug {
		defer func() {
			if err := recover(); err != nil {
				if gogenerateerr, ok := err.(gogenerateerror); ok {
					reterr = gogenerateerr
				} else {
	...
	gogenerate.hashFile(selfHash, "", self)
	gogenerate.selfHash = selfHash.Sum()
	...
	gogenerate.hashFile(goHash, "", gopath)
	gogenerate.goHash = goHash.Sum()
...
func (g *gogenerate) generate(w *pkg) (moreWork []dep) {
	for {
		...
		fmt.Fprintf(hw, "gogenerate %v", g.selfHash)
		...
		for _, d := range w.dirs {
			fmt.Fprintf(hw, "%v\n", d.HashString())
			if d.gen == nil {
				// we simply hash the special gogenerate directive
				continue
			}
	...
	RangeDirs:
		for _, d := range w.dirs {
			if d.gen == nil {
				// special gogenerate directive. We know the only possible command for now
...
func (g *gogenerate) addDep(d dep, nd dep) {
	if *fDebug {
		var count int
...
func (g *gogenerate) dropDep(d dep, od dep) {
...
func (g *gogenerate) doneDep(dd dep) []dep {
	if !dd.Done() {
...
func (g *gogenerate) list(patts []string, opts ...string) ([]*Package, error) {
...
func (g *gogenerate) addDepsFromImports(p *pkg) {
...
func (g *gogenerate) newArchive() *archiveWriter {
...
func (g *gogenerate) cachePutArchive(id cache.ActionID, ar *archiveWriter) error {
...
func (g *gogenerate) isSpecialImport(path string) bool {
	return path == "C"
}

func (g *gogenerate) loadMisses(pkgMisses, dirMisses missingDeps) {
...
func (g *gogenerate) refreshImports(p *pkg, misses missingDeps) {
	var xp *pkg
	xtest := p.Name + "_test"
...
func (g *gogenerate) refreshDirectiveDeps(p *pkg, misses missingDeps) {
	if !p.generate {
	...
	for _, file := range files {
		hasDirPrefix := func(buf []byte) (string, bool) {
			for _, c := range []string{goGeneratePrefix, gogeneratePrefix} {
		...
		err := coregogenerate.DirFuncFunc(p.Name, p.Dir, file, hasDirPrefix, func(prefix string, line int, dirArgs []string) error {
			dir := &directive{
				pkgName: p.Name,
				file:    file,
				line:    line,
...
func (g *gogenerate) hashFile(hw io.Writer, dir, file string) {
	fp := file
...
func (g *gogenerate) hashOutDirs(outDirs []string, p *pkg, outHash io.Writer, dirNames map[string]map[generator]bool) map[string][hashSize]byte {
	...
	if outHash != nil {
		...
		for _, f := range inputFiles {
			...
			for dn := range dirNames {
				if coregogenerate.AnyFileGeneratedBy(f, dn) {
					fhw = newHash("## file " + f)
					ws = append(ws, fhw)
				}
			}
	...
	for _, dir := range outDirs {
		...
		for _, fi := range ls {
			...
			var fhw *hash
			for dn := range dirNames {
				if coregogenerate.AnyFileGeneratedBy(fn, dn) {
					fhw = newHash("## file " + fn)
					ws = append(ws, fhw)
				}
			}
...
func (g *gogenerate) inFiles(p *pkg) []string {
...
func (g *gogenerate) shouldBuildFile(path string) bool {
...
func (g *gogenerate) hashImport(hw io.Writer, path string) {
...
func (g *gogenerate) hashDeps(hw io.Writer, d dep) {
	var deps []dep
...
func (g *gogenerate) fatalf(format string, args ...interface{}) {
	panic(gogenerateerror(fmt.Sprintf(format, args...)))
}

type gogenerateerror string

func (g gogenerateerror) Error() string {
	return string(g)
}

func (g *gogenerate) newPkg(p *Package, createXPkg bool) *pkg {
	res := g.newPkgImpl(p, createXPkg)
	return res
}

func (g *gogenerate) newPkgImpl(p *Package, createXPkg bool) *pkg {
	if _, ok := g.pkgLookup[p.ImportPath]; ok {
...
func (g *gogenerate) resolveGobinModDep(patt string) *gobinModDep {
...
func (g *gogenerate) resolveGobinGlobalDep(patt string) *gobinGlobalDep {
...
func (g *gogenerate) resolveCommandDep(name string) *commandDep {
...
func (g *gogenerate) resolveDir(dir string, dirArgs []string) (generator, error) {
	switch a0 := dirArgs[0]; {
	case a0 == "gobin":
		args := dirArgs[1:]
	...
	case a0[0] == '[':
		// special gogenerate directive is identified by at least one condition
		...
		for strings.HasPrefix(dirArgs[0], "[") && strings.HasSuffix(dirArgs[0], "]") {
			...
			case strings.HasPrefix(cond, "exec:"):
			default:
				return nil, fmt.Errorf("unknown condition in special gogenerate directive")
			}
		}
		if len(dirArgs) != 1 {
			return nil, fmt.Errorf("missing command in special gogenerate directive")
		}
		// If we add further commands, be sure to add the corresponding logic in
		// *gogenerate.generate (which currently assumes only break exists)
		if dirArgs[0] != "break" {
			return nil, fmt.Errorf("unknown command %q in special gogenerate directive", dirArgs[0])
		}
		return nil, gogeneratedir
	default:
		return g.resolveCommandDep(dirArgs[0]), nil
	}
}

var gogeneratedir = errors.New("directive is a special gogenerate directive")
